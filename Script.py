...
root = tk.Tk()
root.overrideredirect(True)  # Remove OS window decorations
...
# 2-pixel gray separator line
separator = tk.Frame(root, bg="gray", height=2)
separator.pack(fill=tk.X)
separator.pack_propagate(False)  # Ensure the height stays at 2 pixels
...
# Second centered text right below the first one
...
# Remove the previous ttk button code
# bottom_button = ttk.Button(root, text="Click Me", style="Custom.TButton")
# bottom_button.pack(side=tk.BOTTOM, pady=20)

# Add a canvas-based button with a thin gray rounded rectangle outline
button_canvas = tk.Canvas(root, width=140, height=40, bg="black", highlightthickness=0)
button_canvas.pack(side=tk.BOTTOM, pady=20)

# Draw a rounded rectangle outline (simulate with create_arc and create_line)
...
# Add button text centered in the rectangle
...
def extract_zip_to_folder(zip_path, target_folder):
    try:
        ...
        # Delete the zip file from temp folder after extraction
        try:
            os.remove(zip_path)
...
# Store offset when dragging starts
def start_move(event):
    root.x_offset = event.x
    root.y_offset = event.y

def move_window(event):
    # Calculate new window position so cursor stays at center of title bar
...
# Bind drag events to title bar, label, and close button
